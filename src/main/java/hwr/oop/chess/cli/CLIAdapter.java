...
public class CLIAdapter {
    ...
    public void createGame(String gameNumber) {
        gameId = Integer.parseInt(gameNumber);
    ...
    private Cell requireArgumentIsCoordinate(Board board, String coordinate) {
        Cell cell = board.findCell(coordinate);
        if (cell != null) {
            cellHighlight.add(cell);
            return cell;
        }
        throw new InvalidUserInputException("The XY-Coordinates must be between A-H and 1-8 (eg. c3, a7). Please correct '" + coordinate + "'.");
    }

    private Figure requireArgumentIsCellWithFigure(Board board, String coordinate) {
        Cell cell = requireArgumentIsCoordinate(board, coordinate);
        return requireCellHasFigure(cell);
    }

    private FigureType requireArgumentIsFigureType(Board board, String figure) {
        return FigureType.fromString(figure);
    }

    private Figure requireCellHasFigure(Cell cell) {
        if (cell.figure() != null) {
            return cell.figure();
        }
        throw new InvalidUserInputException("On the cell " + cell.toCoordinates() + " there is no figure!");
    }
    ...
    private void performActionOnBoardMoveFigure(List<String> arguments) {
        ...
        printlnAction("Move " + figure.color().name() + " " + figure.type().name() + " from " + from.toCoordinates() + " to " + to.toCoordinates() + ".");
    ...
    private void performActionOnBoardPromotePawn(List<String> arguments) {
        ...
        if (figure.type() != FigureType.PAWN) {
            throw new InvalidUserInputException("On the cell " + from.toCoordinates() + " there is no pawn!");
        }

        FigureType promoteToType = requireArgumentIsFigureType(board, arguments.removeFirst());
        printlnAction("Promote " + figure.color().name() + " " + figure.type().name() + " on " + from.toCoordinates() + " to become a " + promoteToType.name() + ".");
    ...
    private void performActionOnBoardShowMoves(List<String> arguments) {
        ...
        Cell from = requireArgumentIsCoordinate(board, arguments.removeFirst());
        Figure figure = requireCellHasFigure(from);
        printlnAction("Showing the cells where the " + figure.type().name() + " on " + from.toCoordinates() + " can move to.");
